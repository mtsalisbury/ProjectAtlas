...
- **The sharper question, replacing last night's version:** tonight's relying-party demo proved a passkey tied to a "Presence" can beat network-based checks — but nothing yet proves that requires *Atlas specifically*. A bare passkey, with no mesh, no context system, no Presence layer at all, might do exactly the same demo equally well. This is the next honest thing to test: what does the Presence layer add on top of a passkey alone that a bank wouldn't already get for free?
...
1. **Answer Tier 3's sharpened question** — what does Presence add beyond a bare passkey. This is now the single most important open question in the whole project.
