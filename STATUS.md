...
**The actual safe operating principle going forward: recovery via the cloud provider's dashboard (Power Cycle/Reboot) works independent of the guest OS's own networking, since it's a hypervisor-level action, not something routed through the box's own network stack.** Any test where a remote node consumes an exit node should be planned expecting to need a power cycle afterward — not as a failure, as a normal part of the test. **Next session's real next step:** retry the Toronto→London test, but immediately after confirming lockout, verify the DNS-based egress check (`dig +short myip.opendns.com @resolver1.opendns.com`) from a device that never loses its own access — either the physically-present Mac (once the separate FetchRIB bug is worked around or fixed upstream) or by accepting the power-cycle-to-recover pattern as expected overhead on any remote consumer node.
