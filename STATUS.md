...
**MFA-home list grew to six banks: added BMO, RBC, Scotiabank, and CIBC alongside TD** — Canada's big five minus National Bank. Same pattern, same caveat every time — base domain only (`bmo.com`, `rbc.com`, `scotiabank.com`, `cibc.com`), relying on `domain_suffix` matching to cover whatever subdomain each bank's actual login page uses, none personally verified against a live login yet, flagged in the file itself exactly like TD already was. Confirmed live in the validated config: `{"domain_suffix": ["td.com", "tdbank.com", "bmo.com", "rbc.com", "scotiabank.com", "cibc.com"], "outbound": "home"}`.
