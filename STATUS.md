...
**MFA-home list now covers Canada's Big Six banks, complete, plus TD's US side.** Added in three passes: BMO/RBC/Scotiabank, then CIBC, then National Bank (`nbc.ca` — Banque Nationale, disambiguated from the US network `nbc.com` by the `.ca` TLD). Same pattern, same caveat every time — base domain only, relying on `domain_suffix` matching to cover whatever subdomain each bank's actual login page uses, none of the six personally verified against a live login yet, flagged in the file itself exactly like TD already was. Confirmed live in the validated config: `{"domain_suffix": ["td.com", "tdbank.com", "bmo.com", "rbc.com", "scotiabank.com", "cibc.com", "nbc.ca"], "outbound": "home"}`.
