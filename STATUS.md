...
**MFA-home list grew to five banks: added BMO, RBC, and Scotiabank alongside TD.** Same pattern, same caveat — base domain only (`bmo.com`, `rbc.com`, `scotiabank.com`), relying on `domain_suffix` matching to cover whatever subdomain each bank's actual login page uses, and none of the three personally verified against a live login yet, flagged in the file itself exactly like TD already was. Confirmed live in the validated config: `{"domain_suffix": ["td.com", "tdbank.com", "bmo.com", "rbc.com", "scotiabank.com"], "outbound": "home"}`.
