...
*(Update, later the same night: this connectivity bug is now understood and fixed — see "SOLVED, same night" below. Both SOCKS proxies confirmed reachable again immediately after. The only remaining blocker for a live end-to-end home-egress test is `sudo` to actually run `sing-box`, plus the still-empty MFA list.)*
